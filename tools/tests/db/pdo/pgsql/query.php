...
foreach ($oResults as $aRow)
{
	...
	$this->isFalse(empty($aRow['q_id']), _WT('The field q_id should not be empty.'));
}

class weePDOResult_pgsql_testForeach extends weeDatabaseRow {}

$oResults = $oDb->query('SELECT * FROM query')->rowClass('weePDOResult_pgsql_testForeach');
foreach ($oResults as $aRow)
{
	$this->isInstanceOf($aRow, 'weePDOResult_pgsql_testForeach', _WT('The returned row should be an instance of weePDOResult_pgsql_testForeach.'));
	$this->isFalse(empty($aRow), _WT('The returned row should not be empty.'));
	$this->isFalse(empty($aRow['q_id']), _WT('The field q_id should not be empty.'));
}
