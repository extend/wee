...
try {
	...
	class weePDOResult_sqlite3_testForeach extends weeDatabaseRow {}

	$oResults = $oDb->query('SELECT * FROM query')->rowClass('weePDOResult_sqlite3_testForeach');
	foreach ($oResults as $aRow)
	{
		$this->isInstanceOf($aRow, 'weePDOResult_sqlite3_testForeach', _WT('The returned row should be an instance of weePDOResult_sqlite3_testForeach.'));
		$this->isFalse(empty($aRow), _WT('The returned row should not be empty.'));
		$this->isTrue(ctype_digit($aRow['q_id']), _WT('The field q_id should not be empty.'));
	}
