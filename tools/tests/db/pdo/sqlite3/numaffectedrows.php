...
try {
	...
	// weePDODatabase::numAffectedRows

	$this->isEqual(1, $oDb->numAffectedRows(),
		_WT('weePDODatabase::numAffectedRows should return 1 when the last query was an INSERT statement.'));

	$oDb->query('UPDATE numaffectedrows SET nar_value=? WHERE nar_value>=80', -1);
	$this->isEqual(20, $oDb->numAffectedRows(),
		_WT('weePDODatabase::numAffectedRows should return the number of updated rows when the last query was an UPDATE statement.'));

	$oDb->query('DELETE FROM numaffectedrows WHERE nar_value!=?', -1);
	$this->isEqual(80, $oDb->numAffectedRows(),
		_WT('weePDODatabase::numAffectedRows should return the number of deleted rows when the last query was a DELETE statement.'));
