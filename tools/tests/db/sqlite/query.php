...
try {
	...
	class weeSQLiteResult_testForeach extends weeDatabaseRow {}

	$oResults = $oDb->query('SELECT * FROM query')->rowClass('weeSQLiteResult_testForeach');
	foreach ($oResults as $aRow)
	{
		$this->isInstanceOf($aRow, 'weeSQLiteResult_testForeach', _WT('The returned row should be an instance of weeSQLiteResult_testForeach.'));
		$this->isFalse(empty($aRow), _WT('The returned row should not be empty.'));
		$this->isTrue(ctype_digit($aRow['q_id']), _WT('The field q_id should not be empty.'));
	}
	...
	$this->isEqual($aAll, $oDb->query('SELECT * FROM query')->fetchAll(),
		_WT('weeSQLiteResult::fetchAll does not return the expected rows of the table.'));
