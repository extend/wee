...
// Test the method weeMySQLiDatabase::escape
...
try {
	$this->isEqual("'7.5'", $oDb->escape(7.5),
		_WT('weeMySQLiDatabase::escape does not return the expected escaped float when the locale is "C".'));

	setlocale(LC_NUMERIC, 'fr_FR');

	$this->isEqual("'7.5'", $oDb->escape(7.5),
		_WT('weeMySQLiDatabase::escape does not return the expected escaped float when the locale is "fr_FR".'));
}
...
// Test the method weeMySQLiDatabase::escapeIdent
...
try {
	...
	$this->fail(_WT('escapeIdent does not throw an InvalidArgumentException when the identifier contains a byte with a value of 255.'));
} catch(InvalidArgumentException $e) {}

try {
	$oDb->escapeIdent('wee ');
	$this->fail(_WT('escapeIdent does not throw an InvalidArgumentException when the identifier end with space characters.'));
} catch(InvalidArgumentException $e) {}

try {
	$oDb->escapeIdent(str_repeat('w', 65));
	$this->fail(_WT('escapeIdent does not throw an InvalidArgumentException when the identifier is longer than 64 characters.'));
} catch(InvalidArgumentException $e) {}
