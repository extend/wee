...
$this->isEqual($aRow['c'], sizeof($aInsertValues),
	_WT('The total number of rows in the table do not match the number of rows inserted.'));
...
$this->isTrue($oResults->valid(),
	_WT('weeMySQLiResult::valid should return true after a call to weeMySQLiResult::rewind when the result set is not empty.'));

$this->isEqual(array('q_id' => 1) + $aInsertValues[0], $oResults->current(),
	_WT('weeMySQLiResult::current should return the first row of the result set after a call to weeMySQLiResult::rewind.'));

$oResults->next();
$this->isEqual(array('q_id' => 2) + $aInsertValues[1], $oResults->current(),
	_WT('weeMySQLiResult::current should return the second row of the result set when moving forward to the second row.'));
...
class weeMysqliResult_testForeach extends weeDatabaseRow {}

$oResults = $oDb->query('SELECT * FROM query')->rowClass('weeMysqliResult_testForeach');
foreach ($oResults as $aRow)
{
	$this->isInstanceOf($aRow, 'weeMysqliResult_testForeach', _WT('The returned row should be an instance of weeMysqliResult_testForeach.'));
	$this->isFalse(empty($aRow), _WT('The returned row should not be empty.'));
	$this->isTrue(ctype_digit($aRow['q_id']), _WT('The field q_id should not be empty.'));
}
...
$this->isEqual($aAll, $oDb->query('SELECT * FROM query')->fetchAll(),
	_WT('weeMySQLiResult::fetchAll does not return the expected rows of the table.'));
