...
if (!class_exists('weePDOResult_oracle_testForeach')) {
	class weePDOResult_oracle_testForeach extends weeDatabaseRow {}
}
...
try {
	...
	$oResults = $oDb->query('SELECT * FROM QUERY')->rowClass('weePDOResult_oracle_testForeach');
	foreach ($oResults as $aRow)
	{
		$this->isInstanceOf($aRow, 'weePDOResult_oracle_testForeach', _WT('The returned row should be an instance of weePDOResult_oracle_testForeach.'));
		$this->isFalse(empty($aRow), _WT('The returned row should not be empty.'));
		$this->isTrue(ctype_digit($aRow['Q_ID']), _WT('The field Q_ID should not be empty.'));
	}
