...
$this->isFalse($oTestEventCached->isEventCached($aEvent),
	sprintf(_WT('The frame %s does not exist.'), $sCacheFilename));
...
$this->isFalse($oTestEventCached->isEventCached($aEvent),
	sprintf(_WT('The page %s has expired.'), $sCacheFilename));

touch($sCacheFilename, time() + 42);
clearstatcache();

$this->isTrue($oTestEventCached->isEventCached($aEvent),
	sprintf(_WT('The page %s should not have expired.'), $sCacheFilename));
