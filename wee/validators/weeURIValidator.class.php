...
/**
	Check if variable passed to the constructor is a valid URI according to the arguments.
*/

class weeURIValidator implements weeValidator
{
	/**
	...
	*/
	...
	protected $aErrorList	= array(
		'invalid'	=> 'Input must be a valid URI'
	);

	/**
		Check if the variable $mValue is an URI according to $aArgs arguments.

		$mValue can be of any type compatible to string.
		$aArgs can contain one of the following keys:
			- invalid_error:	Error message used if the URI given is not valid.
	...
	*/
	...
	public function __construct($mValue, array $aArgs = array())
	{
        ...
        if (!is_string($mValue) || preg_match('/[[:alnum:]]+:\/\/[[:alnum:]-.]+\.[[:alpha:]]{2,4}($|\/)/', $mValue) == 0)
	...
	*/
	...
	/**
		...
		@return	bool	True if value checked is NOT a valid URI, false if it is valid.
	*/
	...
	*/
	...
	/**
		...
		@return	bool	True if $mValue IS a valid URI, false otherwise.
	*/
